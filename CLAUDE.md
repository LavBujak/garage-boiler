...
| `frontend/src/lib/firebase/firestore.ts` | `getUsersCollection()`, `userDoc(uid)`, `getNotesCollection()`, `noteDoc(id)` — add new collections here as `get{X}Collection()` functions (`typedCollection` is module-private) | Typed collection access |
...
| `frontend/src/types/firestore.ts` | `UserProfile`, `Note` — add new collection interfaces here (always with `_schemaVersion: 1`) | Collection types |
...
Pages: `/` · `/auth/signin` · `/auth/signup` · `/dashboard` · `/notes` · `/profile` · `/settings` (route groups `(auth)`, `(dashboard)`). Backend: `GET /api/health` (public); everything else under `/api` requires `Authorization: Bearer <ID token>`.
